...
public class YouTubeManagerController {
	...
	public ResponseEntity<Boolean> favorite(@RequestParam("name") String name, Video video) {
		...
		if (video.getUserList() != null) {
			System.out.println(video.getUserList().toString());
		}
		
		// check if user likes the same video twice
		Video dup = videoService.getVideoById(video.getId(), user.getName());
		if (dup != null) {
			System.out.println("Already Liked this video!");
